...
export class FirebaseService {
  ...
  constructor(private afs: AngularFirestore) {
    this.nombreFilter$ = new BehaviorSubject([""]);
      ...
      switchMap(([nombre, categoria, genero, fabricante]) =>
        afs
          .collection("items", ref => {
            ...
            if (nombre.length <= 1 && nombre[0] === "") {
              nombre = null;
            }

            if (nombre) {
              console.log("buscando..." + nombre.length);
              query = query.where("keywords", "array-contains-any", nombre);
            }
            if (categoria) {
            ...
            if (fabricante) {
              query = query.where("fabricante", "==", fabricante);
            }
  ...
  filterItemsByFabricante(searchterm: string) {
    return this.afs
      .collection("items", ref =>
        ref.where("fabricante", "==", searchterm).limit(10)
      )
      .snapshotChanges()
      .pipe(
        map(c => {
